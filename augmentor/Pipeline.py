...
class Pipeline(object):
    ...
    def __init__(self, source_directory=None, output_directory_parent=None, save_format=None):
        """
        ...
        if output_directory_parent is None:
            output_directory_parent = source_directory[:len(source_directory)-len(parent_directory_name)-1]

        if source_directory is not None:
            self._populate(source_directory=source_directory,
                           output_directory=os.path.join(output_directory_parent, parent_directory_name+'_images_aug/'),
                           ground_truth_directory=None,
                           ground_truth_output_directory=os.path.join(output_directory_parent,
                                                                      parent_directory_name+'_labels_aug/'))
    ...
    def _populate(self, source_directory, output_directory, ground_truth_directory, ground_truth_output_directory):
        ...
        self.augmentor_images, self.class_labels = scan(source_directory, output_directory, ground_truth_output_directory)

        self._check_images(output_directory, ground_truth_output_directory)
    ...
    def _check_images(self, abs_output_directory, ground_truth_output_directory):
        """
        ...
        if len(set(self.class_labels)) <= 1:
            ...
            if not os.path.exists(ground_truth_output_directory):
                try:
                    os.makedirs(ground_truth_output_directory)
                except IOError:
                    print("Insufficient rights to read or write output directory (%s)"
            ...
            for class_label in self.class_labels:
                if not os.path.exists(os.path.join(abs_output_directory, str(class_label[0]))):
                    try:
                        ...
                        print("Insufficient rights to read or write output directory (%s)"
                              % abs_output_directory)
                if not os.path.exists(os.path.join(ground_truth_output_directory, str(class_label[0]))):
                    try:
                        os.makedirs(os.path.join(ground_truth_output_directory, str(class_label[0])))
                    except IOError:
                        print("Insufficient rights to read or write output directory (%s)"
                              % ground_truth_output_directory)
